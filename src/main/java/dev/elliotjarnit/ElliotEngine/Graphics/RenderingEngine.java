...
public class RenderingEngine extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        Matrix3 viewMatrix = scene.getCamera().getViewMatrix();
            ...
            for (EFace face : object.getFaces()) {
                ...
                for (int i = 1; i <= 3; i++) {
                    ...
                    if (point4.x < -aspectRatio || point4.x > aspectRatio || point4.y < -1.0 || point4.y > 1.0) {
                        System.out.println("Point not in view");
                        // Point is not in view
                        continue;
                    }

                    // Raster space
                    point4.x = (point4.x) * getWidth() / 2.0 + getWidth() / 2.0;
                    point4.y = -(point4.y) * getHeight() / 2.0 + getHeight() / 2.0;
